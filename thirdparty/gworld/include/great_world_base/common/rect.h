...
class AxisRange {
 public:
  ...
  void InflateBy(float value) {
    min_ -= value;
    max_ += value;
    min_ = std::min<float>(min_, max_);
    max_ = std::max<float>(min_, max_);
  }
  ...
  float Distance(float pt) const { return std::max<float>(pt - max_, min_ - pt); }
...
class Rect {
 public:
  ...
  void IntersectsRect(const Rect& r) {
      minx_ = std::max<float>(minx_, r.minx_);
      miny_ = std::max<float>(miny_, r.miny_);
      maxx_ = std::min<float>(maxx_, r.maxx_);
      maxy_ = std::min<float>(maxy_, r.maxy_);
  }
  ...
  float Distance(float x, float y) const {
    return std::max<float>(x_.Distance(x), y_.Distance(y));
  }

  void Merge(const Rect& r1, const Rect& r2) {
    minx_ = std::min<float>(r1.minx_, r2.minx_);
    miny_ = std::min<float>(r1.miny_, r2.miny_);
    maxx_ = std::max<float>(r1.maxx_, r2.maxx_);
    maxy_ = std::max<float>(r1.maxy_, r2.maxy_);
  }

  void Merge(const Rect& r) {
    minx_ = std::min<float>(minx_, r.minx_);
    miny_ = std::min<float>(miny_, r.miny_);
    maxx_ = std::max<float>(maxx_, r.maxx_);
    maxy_ = std::max<float>(maxy_, r.maxy_);
  }
  ...
  float get_min(bool gety) const { return gety ? miny_ : minx_; }
  float get_max(bool gety) const { return gety ? maxy_ : maxx_; }
