...
BOOL CBTMgr::init(BOOL bResume)
{
	...
	if (!bResume)
	{
		pMgrData->nBtNodeUsedCount = 1;
		pMgrData->nBtTreeUsedCount = 1;
	}

	//build the index
	for (int32_t nIndex = 0; nIndex < btTotal; nIndex++)
	{
		if (pMgrData->ActionDefList[nIndex].szName[0])
		{
			m_ActionName2Func.insert(std::make_pair(
				pMgrData->ActionDefList[nIndex].szName, pMgrData->ActionDefList + nIndex));
		}
	}
...
int32_t CBTMgr::start_run(BT_CTX& rCtx, int32_t nTreeID, int32_t nEventID, int32_t nOwnerType, void* pOwner, 
	uint64_t qwOwnerID, CLuaScript* pScript, int64_t llEventVar0 /* = 0 */, int64_t llEventVar1 /* = 0 */, int64_t llTriggerVar0 /* = 0 */, int64_t llTriggerVar1 /* = 0 */, BOOL bRollback /* = FALSE */)
{
	int32_t nRetCode = 0;
...
int32_t CBTMgr::_run(BT_CTX& rCtx, void* pOwner, CLuaScript* pScript)
{
	...
	while (rCtx.byCallFrameCount > 0)
	{
		...
		case btSelector:
		{
			...
			{
				DBG("btSelector children count %d", pNode->uChildrenCount);
				...
				else
				{
					...
					DBG("btSelector return %d", rCtx.byReturnValue);
		...
		case btSequencer:
		{
			if (callFrame.dwChild == 0)
			{
				DBG("btSequencer children count %d", pNode->uChildrenCount);
				...
				{
					rCtx.byReturnValue = brvError;
					DBG("btSequncer return %d", rCtx.byReturnValue);
			...
			else
			{
				...
				{
					DBG("sequencer return %d", rCtx.byReturnValue);
		...
		case btParallel:
		{
			if (callFrame.dwChild == 0)
			{
				DBG("batParallel children count %d", pNode->uChildrenCount);
				...
				else
				{
					...
					DBG("batParallel return %d", rCtx.byReturnValue);
		...
		case btRepeat:
		{
			if (callFrame.dwChild == 0)
			{
				if (pNode->uChildrenCount > 0)
				{
					...
					DBG("repeated times %d", pNode->nParam[0]);
					rCtx.byReturnValue = brvError;
				...
				{
					rCtx.byReturnValue = brvSuccess;
					DBG("repeat times %d no child node", pNode->nParam[0]);
			...
			else
			{
				...
				DBG("repeated times [%d | %d]", pNode->nParam[0] - rCtx.byRepeatCounter, pNode->nParam[0]);
		...
		case btRandom:
		{
            if (callFrame.dwChild == 0)
            {
                if (pNode->uChildrenCount >= 1 && pNode->dwChild)
                {
					...
					DBG("btRandom choose child %d on (%d/%d)", nCounter, 
                ...
                else
                {
					...
					DBG("btRandom no child to choose return %d", 
						rCtx.byReturnValue);
            ...
            {
				DBG("btRandom return %d", rCtx.byReturnValue);
...
int32_t CBTMgr::call_node_func(BT_CTX& rCtx, BT_NODE* pNode)
{
	...
	if (rCtx.bDebugEnabled || m_pMgrData->bDebugEnabled)
	{
		char szNodeInfo[BT_DEBUG_INFO_LEN];
		for (int32_t i = 0; i < (rCtx.byCallFrameCount - 1) * 3; i++)
			szNodeInfo[i] = ' ';
		szNodeInfo[(rCtx.byCallFrameCount - 1) * 3] = 0;

		INF("%s%s (%d %d %d %d %d %d) : %s", szNodeInfo, get_action_name(pNode->uType),
			nParam[0], nParam[1], nParam[2], nParam[3], nParam[4], nParam[5]);
	}
...
int32_t CBTMgr::register_action(const char* pcszName)
{
...
int32_t CBTMgr::create_bt_tree(uint32_t dwRootNodeID, CLuaScript* pScript)
{
	...
	{
		INF("create bt tree success script %s for tree %d has exist", pScript->get_script_name(), it->second->nTreeID);
