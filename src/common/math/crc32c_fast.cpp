...
extern "C" uint32_t crc32c_append_hw(uint32_t crc, buffer buf, size_t len)
{
    buffer next = buf;
    buffer end;
#ifdef _M_X64
...
extern "C" int crc32c_hw_available()
{
    int bSupport = 0;
...
extern "C" uint32_t crc32c_append(uint32_t crc, buffer input, size_t length)
{
	if (append_func == NULL)
	{
		_crc32_init();
